...
RCT_EXPORT_METHOD(poiSearch:(CLLocationCoordinate2D) pt option: (NSDictionary*) opt)
{
    ...
    options.location = pt;
    BOOL flag = [[self getPoiSearch] poiSearchNearBy:options];
    if(flag) {
        NSLog(@"poi search成功");
    }
}
...
- (void)onGetPoiResult:(BMKPoiSearch*)searcher result:(BMKPoiResult*)result errorCode:(BMKSearchErrorCode)error
{
    ...
    if (error == BMK_SEARCH_NO_ERROR) {
        body[@"pois"]=  [GeolocationModule _convertPoiList: result.poiInfoList];
        body[@"bianbian"]= @"bianbina";
    }
    else {
        body[@"errcode"] = [NSString stringWithFormat:@"%d", error];
        body[@"errmsg"] = [self getSearchErrorInfo:error];
...
-(void) onGetReverseGeoCodeResult:(BMKGeoCodeSearch *)searcher result:(BMKReverseGeoCodeResult *)result
    ...
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        body[@"pois"]=  [GeolocationModule _convertPoiList: result.poiList];
...
+ (NSArray *) _convertPoiList: (NSArray*) list
{
    ...
    for (size_t i = 0; i<list.count; ++i) {
        ret[i] = [GeolocationModule _convertPoi:list[i]];
    }
    return ret;
}
